    ...
    return (
        <>
            <div className='user-top-nav'>
                <div className={`top-nav-items ${activePage === `/${localStorage.getItem("userId")}/profile/details` ? 'small-active-page' : ''}`} onClick={() => handleNavigation(`/${localStorage.getItem("userId")}/profile/details`)}>
                    <div><i className="bi bi-person side-nav-icon"></i></div>
                    <div>Details</div>
                </div>
                <div className={`top-nav-items ${activePage === `/${localStorage.getItem("userId")}/profile/orders` ? 'small-active-page' : ''}`} onClick={() => handleNavigation(`/${localStorage.getItem("userId")}/profile/orders`)}>
                    <div><i className="bi bi-receipt-cutoff side-nav-icon"></i></div>
                    <div>Orders</div>
                </div>
                <div className={`top-nav-items ${activePage === `/${localStorage.getItem("userId")}/profile/reviews` ? 'small-active-page' : ''}`} onClick={() => handleNavigation(`/${localStorage.getItem("userId")}/profile/reviews`)}>
